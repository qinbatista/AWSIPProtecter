    ...
    def _update_this_server(self):
        ...
        while True:
            try:
                ...
                target_ipv4 = gethostbyname(self.__target_server)
                self._can_connect = 0
                udpClient.sendto((f"{target_ipv4},{str(self._can_connect)}").encode(encoding="utf-8"), (self.__target_server, 7171))
                print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}][update_this_server]Updated to {target_ipv4}, message={this_docker_ipv4},{self._can_connect}")
                time.sleep(10)
